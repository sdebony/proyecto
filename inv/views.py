...
class ProductoView(SinPrivilegios, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        print("Estos son productos subcategoria != 2")
        prod = Producto.objects.filter(subcategoria=1).all()
        context={'obj':prod}
        return context
...
class TubosView(SinPrivilegios, generic.ListView):
    model = Tubos
    template_name = "inv/tubos_list.html"
    context_object_name = "obj"
    permission_required="inv.view_producto"
    form_class=TubosForm
    success_url= reverse_lazy("inv:tubos_list")
    success_message="Lista de tubos"
    ...
    def get_context_data(self, **kwargs):
        ...
        print("Tubos")
        tubos = Tubos.objects.all()
        context={'obj':tubos}
        return context

class TubosEdit(SuccessMessageMixin,SinPrivilegios,
                   generic.UpdateView):
    model=Tubos
    template_name="inv/tubos_form.html"
    context_object_name = 'obj'
    form_class=TubosForm
    success_url= reverse_lazy("inv:tubos_list")
    success_message="Tubo Editado"
    permission_required="inv.change_producto"
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super(TubosEdit, self).get_context_data(**kwargs)
        context["obj"] = Tubos.objects.filter(pk=pk).first()

        return context

class TubosNew(SuccessMessageMixin,SinPrivilegios,
                   generic.CreateView):
    model=Tubos
    template_name="inv/tubos_form.html"
    context_object_name = 'obj'
    form_class=TubosForm
    success_url= reverse_lazy("inv:tubos_list")
    success_message="Tubo Creado correctamente"
    permission_required="inv.add_producto"
    ...
    def get_context_data(self, **kwargs):
        context = super(TubosNew, self).get_context_data(**kwargs)
        return context
